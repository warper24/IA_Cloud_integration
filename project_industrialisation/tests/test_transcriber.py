...
@pytest.mark.parametrize("invalid_file", ["audio.mp3", "audio.flac", "audio.ogg", "audio.txt"])
...
def test_validate_audio_file_valid(transcriber):
    try:
        ...
        pytest.fail("validate_audio_file a levé une ValueError pour un .wav valide")
...
def test_transcribe_failure(transcriber):
    with patch.object(transcriber, "model", side_effect=Exception("Erreur de transcription")):
        result = transcriber.transcribe("test.wav")
